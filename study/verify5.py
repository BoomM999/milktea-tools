...
# 配料表制作文本本身就是带换行的字符串 (since we read with \n from multi-line cell)
# Verify: split ingredient 制作 by newline -> should match pwd segments split by |
# Need to find ingredient 制作 text. Let's locate via search across all sheets.
def find_make(sheet_rows, name, size_kw):
    cur=None
    for r in sheet_rows:
        c0=(r[0] if len(r)>0 else '').strip()
        c1=(r[1] if len(r)>1 else '').strip()
        c2=(r[2] if len(r)>2 else '').strip()
        if c0==name and size_kw in c1 and c2:
            return c1,c2
    return None,None
...
for sheet in ing.values():
    for nm,sz in checks:
        res=find_make(sheet,nm,sz)
        if res[0] is not None:
            spec,mk=res
            pwd=mp_lookup.get((nm,sz),{}).get('标准糖')
            print(f"\n■ {nm}/{sz}  (sheet found)")
            print(f"   配料杯型: {spec!r}")
            print(f"   配料制作(raw, newlines shown as /):")
            for li,line in enumerate(mk.split('\n')):
                print(f"     [{li}] {line!r}")
            if pwd:
                print(f"   密码段(split |):")
                for si,seg in enumerate(pwd.split('|')):
                    print(f"     [{si}] {seg!r}")
            else:
                print(f"   密码: None")
            checks.remove((nm,sz))
print("\nUnmatched in ingredient sheets:", checks)
